...
local function load_adapter()
    local name = (cfg and cfg.rerank_adapter) or "noop"
    local ok, mod = pcall(require, "lapis_memory.rerankers." .. name)
    if not ok then
...
function M.rerank(query, hits, opts)
    ...
    -- Temporarily swap the orchestrator's cfg for adapter loading.
    local saved_cfg = cfg
    cfg = view
    local adapter, aerr = load_adapter()
    cfg = saved_cfg
